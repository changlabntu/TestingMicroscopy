...
from networks.EncoderDecoder.ed023e import conv3d_bn_block as conv3_block
from networks.EncoderDecoder.ed023e import deconv3d_bn_block
...
class AE(nn.Module):
    def __init__(self, ddconfig, nf=32, act=nn.ReLU, norm_type='group', dropout=0.5, final='tanh', out_channels=1):
        super().__init__()
        ...
        self.conv7_k = nn.Sequential(
            conv3_block(nf * 2, out_channels, activation=final_layer, norm='none'),
        )
        ...
        self.decode_up4 = deconv3d_bn_block(4 * nf, 2 * nf, use_upsample=(1, 1, 4), activation=act)
        self.decode_up8 = deconv3d_bn_block(2 * nf, 1 * nf, use_upsample=(1, 1, 8), activation=act)
    ...
    def forward(self, x, method=None):
        if method != 'decode':
            ...
            feat = [xs[1], xs[3], xs[5], x]
            feat = [y.permute(1, 2, 3, 0).unsqueeze(0) for y in feat]  # (1, C, X, Y, Z)
...
if __name__ == "__main__":
    ...
    Generator = getattr(getattr(__import__('networks.ldm.' + 'aeunet'), 'ldm'), 'aeunet').AE
    net = Generator(ddconfig)

    z = net(torch.rand(1, 1, 128, 128, 16), method='encode')
    #print(z.shape)
