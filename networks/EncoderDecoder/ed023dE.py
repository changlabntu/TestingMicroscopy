...
def deconv3d_bn_block(in_channels, out_channels, use_upsample=(2, 2, 2), kernel=3, stride=1, padding=1, momentum=0.01,
                      activation=ACTIVATION):
...
class Generator(nn.Module):
    def __init__(self, n_channels=1, out_channels=1, nf=32, norm_type='batch', activation=ACTIVATION, final='tanh', mc=False):
        ...
        self.up3 = deconv3d_bn_block(8 * nf, 4 * nf, activation=act)#, kernel=4, stride=2, padding=1)
        ...
        self.up2 = deconv3d_bn_block(4 * nf, 2 * nf, activation=act)#, kernel=4, stride=2, padding=1)
        ...
        self.up1 = deconv3d_bn_block(2 * nf, nf, activation=act)#, kernel=4, stride=2, padding=1)
